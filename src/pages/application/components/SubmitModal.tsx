...
import { SUBMIT_MODAL, SUCCESS_MODAL } from '../constants/index';
...
interface SubmitModalProps {
  ...
  onConfirm: () => void;
  partName: string;
  submitStatus: 'idle' | 'loading' | 'success' | 'error'; // 상태 추가
}
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 px-4">
      <div className="relative flex w-full max-w-[43.4375rem] flex-col rounded-[1.25rem] bg-[#F7FAFF] p-8 shadow-[0_0_22.7px_0_rgba(27,38,52,0.13)] md:p-12 lg:p-[5rem_3.25rem]">
        {/* 닫기 버튼 */}
        {!isSuccess && (
          <button onClick={onClose} className="absolute top-6 right-6 cursor-pointer lg:top-[3rem] lg:right-[3.25rem]">
            <img src={CloseIcon} alt="close" className="h-[1rem] w-[1rem] shrink-0 md:h-[1.125rem] lg:h-[1.28913rem]" />
          </button>
        )}
        ...
        <div className="flex flex-col items-center gap-8 lg:gap-[2.375rem]">
          ...
          <div className="flex w-full flex-col gap-6 lg:gap-[2rem]">
            ...
            <h2 className="text-[1.75rem] font-semibold break-keep text-[var(--color-navyblack)] md:text-[2rem] lg:text-[2.25rem]">
              {isSuccess ? SUCCESS_MODAL.TITLE : SUBMIT_MODAL.TITLE}
            </h2>
          ...
          {/* 버튼 영역 */}
          <button
            onClick={isSuccess ? () => navigate('/home') : onConfirm}
            disabled={isLoading}
            className={`group flex items-center justify-center gap-[0.625rem] rounded-[1rem] border-[1.5px] border-[#4284FF] bg-[#F7FAFF] px-[1rem] py-[0.5rem] md:px-[1.875rem] md:py-[0.9375rem] lg:px-[1.875rem] lg:py-[0.9375rem] ${isLoading ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-[#4284FF]'} `}>
            <span
              className={`text-[1rem] font-bold md:text-[1.375rem] lg:text-[1.5rem] ${isLoading ? 'text-[#4284FF]' : 'text-[#4284FF] group-hover:text-white'} `}>
              {isLoading ? '제출 중...' : isSuccess ? SUCCESS_MODAL.BUTTON : SUBMIT_MODAL.BUTTON_TEXT}
            </span>
          </button>
        </div>
      </div>
