...
const SCHEDULE_SECTION_STYLES = {
  section: {
    base: 'scrollbar-hidden relative flex flex-col justify-center items-center overscroll-contain',
  ...
  scheduleItem: {
    base: 'relative flex flex-col w-full snap-center  [scroll-snap-stop:always] items-center',
    desktop: 'lg:mt-[10rem]',
    tablet: 'md:mt-[5rem]',
    mobile: 'mt-[3rem]',
  },
...
const ScheduleSection = ({ scrollContainerRef }: { scrollContainerRef: React.RefObject<HTMLDivElement | null> }) => {
  const resistanceRef = useScrollResistance(0.3, scrollContainerRef.current); // 0.15(매우 무거움) ~ 0.4(거의 정상) 사이로 저항감 조정
